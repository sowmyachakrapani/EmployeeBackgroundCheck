...
namespace EmpBackgroundVerify
{
    public class DB2
    {
        ...
        public bool IsValidLogin_Employee(string id, string pswd)
        {
        ...
        #region Get All companies details
        public DataTable Company_GetAll_Approved2()
        {
        ...
        //    return b.DoTransaction(sql);
        //}
        public int Employee_Add_New_Basic(string name, string address, string mobileno, string emailid,string dob, string quali, string cardno,string pswd)
        {
            string sql = string.Format("insert into tblEmployees (EmpName,Address,MobileNo,EmailId,DOB,Qualification,AadharCardNo,Date,Password) values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')", name, address, mobileno, emailid,dob, quali, cardno, DateTime.Now.ToShortDateString(),pswd);
            return b.DoTransaction(sql);
        }
        #endregion
        ...
        #region get employee details based on empid
        public DataTable Emp_Get_EmpId(int eid)
        {
            string sql = string.Format("select * from tblEmployees where EmpId={0}",eid);
            return b.DoNonTransaction(sql);
        }
        #endregion

        #region admin change passsword
        public int Emp_ChangePassword(string pswd, int eid)
        {
            string sql = string.Format("update tblEmployees set Password='{0}' where EmpId={1}",pswd,eid);
            return b.DoTransaction(sql);
        }
        #endregion

        #region Get employee working company details
        public DataTable Emp_CompanyId(int eid)
        {
            string sql = string.Format("SELECT  tblEmployees.*, tblEmployeeDetails.CompanyId, tblCompanies.CompanyName FROM tblCompanies INNER JOIN tblEmployeeDetails ON tblCompanies.CompanyId = tblEmployeeDetails.CompanyId INNER JOIN tblEmployees ON tblEmployeeDetails.EmpId = tblEmployees.EmpId where tblEmployees.EmpId={0} and  tblEmployeeDetails.Status='Working'",eid);
            return b.DoNonTransaction(sql);
        }
        #endregion

        #region fetch company details 
        public DataTable Emp_GetCompanies(string compid)
        {
            string sql = string.Format("select * from tblCompanies where CompanyId not in ('{0}')",compid);
            return b.DoNonTransaction(sql);
        }

        #endregion

        #region employee uploading resume
        public int Emp_UploadResume(string cid, int eid, string path)
        {
            string sql = string.Format("insert into tblEmployeeResumes (CompanyId,EmpId,Resume,Date) values ('{0}',{1},'{2}','{3}')",cid,eid,path,DateTime.Now.ToShortDateString());
            return b.DoTransaction(sql);
        }
        #endregion

        #region fetch employee resume based on empId and compId
        public DataTable Emp_GetResume_Cid_Eid(string cid, int eid)
        {
            string sql = string.Format("select * from tblEmployeeResumes where CompanyId='{0}' and EmpId={1}", cid, eid);
            return b.DoNonTransaction(sql);
        }
        #endregion


        #region update resume based on empId and compId
        public int Emp_GetResume_Cid_Eid(int r_id,string path)
        {
            string sql = string.Format("update tblEmployeeResumes set Resume='{0}',Date='{1}' where ResumeId={2}",path, DateTime.Now.ToShortDateString(),r_id);
            return b.DoTransaction(sql);
        }
        #endregion
        ...
        #region fetch employee resume details based on compid
        public DataTable EmpResume_GetAll_CompId(string cid)
        {
            string sql = string.Format("select * from tblEmployeeResumes where CompanyId='{0}'",cid);
            return b.DoNonTransaction(sql);
        }
        #endregion

        #region fetch employee resume details based on compid and empid
        public DataTable EmpResume_GetAll_CompId_EmpId(string cid, int eid)
        {
            string sql = string.Format("select * from tblEmployeeResumes where CompanyId='{0}' and EmpId={1}", cid,eid);
            return b.DoNonTransaction(sql);
        }
        ...
        #region get employees details based on employeeid
        public DataTable EmpResume_GetEmpDetails_EmpId(int empid)
        {
            string sql = string.Format("SELECT tblDepartments.DeptName,tblDepartments.DeptId,tblDesignations.Designation,tblDesignations.DesId, tblEmployeeDetails.DOJ,tblEmployeeDetails.CompanyId, tblSalaries.CTC, tblSalaries.HikeDate, tblSalaries.Rating AS SalaryRating, tblSalaries.Status AS SalaryStatus,tblEmployeeDetails.Status AS WorkStatus,tblEmployeeDetails.QuitDate,tblEmployeeDetails.QuitReason,tblEmployeeDetails.Rating AS OverAllRating,tblCompanies.CompanyName FROM tblDesignations INNER JOIN tblDepartments INNER JOIN tblSalaries INNER JOIN tblEmployees ON tblSalaries.EmpId = tblEmployees.EmpId INNER JOIN tblCompanies ON tblSalaries.CompanyId = tblCompanies.CompanyId ON tblDepartments.CompanyId = tblCompanies.CompanyId ON  tblDesignations.CompanyId = tblCompanies.CompanyId INNER JOIN tblEmployeeDetails ON tblDesignations.DesId = tblEmployeeDetails.DesId AND tblDepartments.DeptId = tblEmployeeDetails.DeptId AND tblEmployees.EmpId = tblEmployeeDetails.EmpId where tblEmployeeDetails.EmpId={0}", empid);
            return b.DoNonTransaction(sql);
        }
        #endregion
